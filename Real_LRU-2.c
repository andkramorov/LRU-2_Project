...
int main() {
    ...
    int size, number, i, hits, element, counter, hash, victim, max_tree_size, buffer_counter, correl_period_of_refd_page;
    struct HashTab *hashtab;
    long long data_storage;
    void * data;
        ...
        if(check.True == 1) {
            if(check.value->address == 0) {
                ...
                //inserting in buffer
                    ...
                    assert(data);
                    *(long long * )check.value->address = data;
                    buffer_counter++;
                }
                tree_update(t, check.value, check.value->hist1);
                check.value->hist1 = i;
                check.value->last = i;
            }
            else {
